...
Similar to the other visualization functions, `Utility_ThirdColor()` plots the data from the specified GatingSet and subset for the given X and Y axis-markers. From here it will overlay a color gradient for an additional marker, allowing for it's visualization. 

```{r}
SinglePlot <-  Utility_ThirdColorPlots(x=UnmixedGatingSet[1], subset = "live", xaxis="BUV496-A", yaxis = "Spark Blue 550-A",  zaxis ="BUV805-A", splitpoint = "130", sample.name = "GROUPNAME", removestrings = c("DTR", ".fcs"), thecolor = "orange")
...
`Utility_ThirdColor()` has three plotting modes. When "Continous" is specified, it takes two colors and forms a color gradient from the lowest to highest values specified for the parameter specified in the zaxis argument. When "Categorical" is specified, it converts the zaxis column into a factor, and plots the factor(s) specified in the argument. The third option "splitpoint" will dichotomize the continous marker expression at the given value. 
