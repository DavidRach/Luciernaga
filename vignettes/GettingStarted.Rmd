...
abstract: |
  Spectral Flow Cytometry (SFC) ability to resolve similar fluorophores is dependent on it's unmixing controls (both single color and unstained). Luciernaga is a collection of tools to enable individual users to profile quality of their unmixing controls and their subsequent effects on unmixing of full-stained samples.
...
Despite their critical role, there are few resources and tools (either open-source or commercial) available to evaluate the quality of the unmixing controls. This has led to individual users only becoming aware of an issue after unmixing the full-stained sample with the unmixing looking "off", leading to a reading of tea leaves in attempt to parse what may have led to the outcome. 

Given how multiple autofluorescences, tandem degradation, or instrumental error can on their own contribute to loss of resolution, a way to highlight problematic unmixing controls quickly and evaluate their potential effect on re-unmixing would be useful. 

Luciernaga is an R package that attempts to address this gap. It provides functionality to implement and tailor quality control checks of unmixing controls, characterize normalized signatures present, and evaluate their effect on unmixing. Using functional programming, it can enable data exploration and visualization for an entire data set. 
