...
# Introduction
Spectral flow cytometry (SFC) is heavily dependent on the quality of it's unmixing controls (both single-color (SC) and unstained (UC)) to resolve the presence and relative abundance of fluorophores on individual cells in the full-stained sample. Despite this critical role, there is a scarcity of tools and resources to check quality control of these unmixing controls in either open-source or commercial software. The individual user is only alerted to issues when the final unmixed .fcs file looks off, and is left to parse out whether the issue is multiple autofluorescences, tandem degradation, or instrumental error on their own by individually replacing individual unmixing controls and attempting to re-unmix again and see if the issue has resolved. 

Our R package Luciernaga is aimed at addressing this gap. It provides functions to perform quality control checks of unmixing controls, interrogate normalized signatures present within individual .fcs files, and visualize the resulting data. 
...
```{r}
SingleUnityPlot <- Utility_UnityPlot(x="Spark Blue 550-A", y="BUV805-A", GatingSet=UnmixedGatingSet, sample.name="GROUPNAME", bins=100, clearance=0.2,removestrings=removestrings, marginsubset="lymphocytes", gatesubset="live", gatelines=FALSE, reference=NULL, pdf=FALSE, outpath=StorageLocation)
...
MultipleUnityPlots <- map(.x=KeptMarkers[c(1:6, 8:10)], .f=Utility_UnityPlot, y="BUV805-A", GatingSet=UnmixedGatingSet, sample.name="GROUPNAME", bins=100, clearance=0.2,removestrings=removestrings, marginsubset="lymphocytes", gatesubset="live", gatelines=FALSE, reference=NULL, pdf=FALSE, outpath=StorageLocation)
...
To provide multiple arguments to `Utility_ThirdColorPlots()`, a data.frame can be provided to the splitpoint argument. It will then iterate down the rows, returning only cells that are greater than values listed for each marker. 

```{r}
FileLocation <- system.file("extdata", package = "Luciernaga")
CSVLocation <- file.path(path = FileLocation, pattern = 'ThirdColorDataFrame.csv')
read.csv2(CSVLocation)
Hmm <- read.csv(file.path(path = FileLocation, pattern = 'ThirdColorDataFrame.csv'))

```



"splitpoint" can also cross-match the gs specimen to the Coereba packages designated splitpoint for that specific marker in a designated .csv file, allowing equivalent plotting across specimens that may have batch effects. A further advance use, when splitpoint is given a data.frame, it will iterate down the rows, subfiltering from the remaining cells as it descends. 

The default is to return ggplot objects, which in turn can be arranged using `Utility_Patchwork()` into a .pdf of desired layout. Combining with map function, it's possible to extend the visualization capacity for many scenarios similar to Utility_UnityPlots and Utility_NxNPlots. As community usefulness is demonstrated, these will become integrated in later versions of the package. 
