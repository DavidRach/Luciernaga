...
InternalUnmix <- function(cs, StashedDF, TheData, Ligands) {
  ...
  for (i in new_pid) {
    ...
    TheName <- new_pd[[i, 1]]
    if (!str_detect(TheName, "Time|FSC|SSC")) {
      Test[paste0(i,"V")] <- "0"
    } else {
    ...
    if (!str_detect(TheName, "FSC|SSC")) {
      ...
      Test[paste0(NoDollar,"DISPLAY")] <- "LIN"
    }

    if (str_detect(TheName, "Time")) {
      Test[paste0(i,"TYPE")] <- "Time"
      ...
      Test[paste0(i,"TYPE")] <- "Unmixed_Fluorescence"
    }
  }
