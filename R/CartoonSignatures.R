...
CartoonSignatures <- function(x = NULL, columnname = "Sample", data, 
  ...
  if (is.null(Expand) && returnType %in% c("Density", "Ribbon", "Amalgamated")) {
    Expand <- 100
  }

  if (!is.null(ylim)) {
    if (!is.numeric(ylim)) stop("`ylim` must be a numeric vector.")
    if (length(ylim) == 1) ylim <- c(0, ylim)
    else if (length(ylim) != 2) stop("`ylim` must be a numeric vector of length 2, e.g., c(0, 100).")
  }

  if (is.null(x)) x <- data |> dplyr::pull(columnname)

  StartingData <- data |> dplyr::filter(.data[[columnname]] %in% x)
  CharacterLength <- StartingData |> dplyr::select(!tidyselect::where(is.numeric)) |> length()

  if (CharacterLength == 0) stop("Please add a non-numeric column, and provide its columnname")
  ...
  if (CharacterLength > 1 && !is.null(characterColumns)) {
    Identity <- StartingData |> 
      ...
      dplyr::pull()
    Identity <- data.frame(Fluorophore = Identity) |> dplyr::rename("Fluorophore" = 1) |> dplyr::mutate(Fluorophore = paste0("ID_", Fluorophore))
  } else {
    Identity <- StartingData |> dplyr::select(!tidyselect::where(is.numeric)) |> dplyr::rename("Fluorophore" = 1) |> dplyr::mutate(Fluorophore = paste0("ID_", Fluorophore))
  }
  ...
  if (!is.null(Expand) && is.numeric(Expand) && Expand > 0) { 
    if (is.character(variance)) {
      variance <- match.arg(tolower(variance), choices = c("none", "low", "medium", "high"))
      ...
      if (length(variance) == 1) {
        var_range <- if (variance == 0) c(0.999, 1.001) else c(1 - abs(variance), 1 + abs(variance))
      ...
      stop("`variance` must be a preset string ('none', 'low', 'medium', 'high') or numeric.")
    }

    idx <- rep(seq_len(nrow(DetectorCols)), each = Expand)
    Identity <- Identity |> dplyr::slice(idx) |> dplyr::mutate(Fluorophore = paste0(Fluorophore, "_", dplyr::row_number()))
    DetectorCols <- DetectorCols |> dplyr::slice(idx)
    
    row_scales <- runif(nrow(DetectorCols), min = var_range[1], max = var_range[2])
    channel_dev <- (var_range[2] - 1) * 0.3
    channel_noise <- matrix(
      runif(prod(dim(DetectorCols)), min = 1 - channel_dev, max = 1 + channel_dev),
  ...
  WhoseThis1 <- WhoseThis |>
    tidyr::pivot_longer(cols = tidyselect::where(is.numeric), names_to = "Detector", values_to = "AdjustedY")

  amalgamate_input <- WhoseThis |> dplyr::mutate(Count = 1) |> dplyr::relocate(Count, .after = Fluorophore)

  if (returnType == "Cartoon") {
    ThePlot <- CartoonInternal(TheseFluorophores = TheseFluorophores, TheFluorophore = NULL, 
                               data = WhoseThis1, legend = legend, plotname = plotname, 
    ...
    ThePlot <- QC_Amalgamate(data = amalgamate_input, samplecolumn = "Fluorophore", 
                             countcolumn = "Count", normalize = FALSE, returnType = "plot", 
                             titlename = plotname, linecolor = line_col, legend = legend)
    ...
    plot_df <- WhoseThis1 |> 
      dplyr::mutate(DetectorIdx = as.numeric(factor(Detector, levels = detector_names)))
    
    if (isTRUE(logScale)) plot_df <- plot_df |> dplyr::filter(AdjustedY > 0)
    ...
    for (s in seq_len(n_shells)) {
      ...
      y_low_interp <- stats::approx(q_per_det$DetectorIdx, q_per_det$y_low, xout = interp_x)$y
      y_high_interp <- stats::approx(q_per_det$DetectorIdx, q_per_det$y_high, xout = interp_x)$y
    ...
    median_per_det <- plot_df |>
      dplyr::group_by(DetectorIdx) |>
      dplyr::summarise(mid = median(AdjustedY, na.rm = TRUE), .groups = "drop")
    
    median_interp <- data.frame(
      x = interp_x,
      y = stats::approx(median_per_det$DetectorIdx, median_per_det$mid, xout = interp_x)$y
    )
    ...
    ThePlot <- ThePlot +
      ...
      ggplot2::labs(
        title = if (!is.null(plotname)) plotname else "Fluorophore Density Ribbon",
        x = "Detector",
        y = "Intensity"
      ) +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust = 1))

    if (!legend) ThePlot <- ThePlot + ggplot2::theme(legend.position = "none")
    if (isTRUE(logScale)) ThePlot <- ThePlot + ggplot2::scale_y_log10()
    if (!is.null(ylim)) ThePlot <- ThePlot + ggplot2::coord_cartesian(ylim = ylim)

  } else {
    stop("Invalid returnType. Options are: 'Cartoon', 'Amalgamated', 'Density', or 'Ribbon'.")
  }
