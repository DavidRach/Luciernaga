...
#' @importFrom dplyr bind_cols select mutate n
...
#'
#' ff <- gs_pop_get_data(
#'   UnmixedGatingSet[1], subsets = "live", inverse.transform = FALSE
#' )
#' BeforeParameters <- ff[[1, returnType = "flowFrame"]]
#' MainDataFrame <- as.data.frame(exprs(ff[[1]]), check.names = FALSE)
#' NewData <- MainDataFrame |>
#'   mutate(ExposureStatus = sample(1:3, n(), replace = TRUE))
#' NewData <- NewData |> select(ExposureStatus)
#' AfterParameters <- Utility_ColAppend(
#'   ff = ff, DF = MainDataFrame, columnframe = NewData
#' )
#'
Utility_ColAppend <- function(ff, DF, columnframe, shift = FALSE) {
  ...
  if (shift == TRUE) {
    NotNegativeColumns <- map(
      .x = TheColNames, .f = InternalShift, columnframe = columnframe
    ) |>
      bind_cols() # Values not below
  } else {
    NotNegativeColumns <- columnframe
  }
