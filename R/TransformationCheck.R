...
TransformationCheck <- function(x, thesecolumns = NULL,
                                ...
                                channelRange = 256, maxValue = 1000000,
                                pos = 4.5, neg = 1, widthBasis = -500,
                                Multiple = FALSE, returnType = "NxN",
                                optionalY = "BUV805-A",
                                optionalX = "BUV496-A",
  ...
  if (class(x) == "character") {
    MyCytoSet <- load_cytoset_from_fcs(
      x, transformation = FALSE, truncate_max_range = FALSE
    )
  ...
  if (!is.null(thesecolumns)) {
    Retained <- KeptMarkers %in% thesecolumns
  } else {
    Retained <- KeptMarkers
  }
  ...
  if (Multiple == FALSE) {
    Returns <- map(
      .x = InternalGS, .f = InternalTransformation,
      TransformationChoice = TransformationChoice,
      channelRange = channelRange, maxValue = maxValue,
      pos = pos, neg = neg, widthBasis = widthBasis,
      KeptMarkers = KeptMarkers
    )
    Flag <- FALSE
  } else {
    Returns <- InternalTransformation(
      x = InternalGS, TransformationChoice = TransformationChoice,
      channelRange = channelRange, maxValue = maxValue,
      pos = pos, neg = neg, widthBasis = widthBasis,
      KeptMarkers = KeptMarkers
    )
  ...
  if (returnType == "GatingSet") {
    ...
    if (Flag == TRUE) {
      ...
      message("Combining Individual GatingSets")
    }
    ...
    if (Flag == TRUE) {
      ...
      if (is.null(optionalOutpath)) {
        ...
        outpath <- optionalOutpath
      }

      Plot <- Utility_GatingPlots(
        x = Returns[1], sample.name = c("GROUPNAME", "TUBENAME"),
        removestrings = ".fcs", subset = "root", gtFile = NULL,
        DesiredGates = NULL, outpath = outpath, returnType = "pdf",
        therows = 1, thecolumns = 1, optionalY = optionalY,
        optionalX = optionalX, filename = optionalName
      )
      return(Plot)
    } else {
      message("Returning Multiple Plots")
      ...
      Plots <- map(
        .x = Returns, .f = Utility_GatingPlots,
    ...
    if (Flag == FALSE) {
      stop("Rerun with Multiple set to True")
    }
    message("Option Unity plot selected for returnType")
  } else {
    message("No option selected for returnType")
  }
}