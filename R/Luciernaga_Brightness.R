#' Visualize MFI of raw .fcs files to evaluate single color
#'  controls.
#'
#' @param fluorophore.name The name of the fluorophore to filter
#'  from data
#' @param data The data.frame derrived by Luciernaga_FolderBrightness,
#'  consisting of fluorophore and cluster metadata, and the raw
#'  detector values
#' @param fluorophore.column The name of the column containing the
#'  fluorophore  information
#' @param cluster.column The name of the column containing the cluster
#'  information
#' @param downsample Default is TRUE, to avoid having large populations
#'  take over the entire y-axis.
...
#'  use same naming as with the passed fluorophore argument in x
#' @param clearance A buffer area multiplier to xmin and xmax when
#'  scaled equals false
#' @param Scaled Default is set to TRUE, returning log_10 transformed
#'  data, FALSE will return raw MFI.
