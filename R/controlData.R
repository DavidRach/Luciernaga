...
#' @param removestrings Portions of the single color names you want removed (
#' Ex. c("DR_ILT_2023_", ".FCS")
#'
...
controlData <- function(cs, removestrings) {
  ...
  Control_Spectrums<-Control_Spectrums[,grep("SC|SS|FS", names(Control_Spectrums),
                                             invert=TRUE)]
  row.names(Control_Spectrums) <- NameCleanUp(row.names(Control_Spectrums),
                                              removestrings)
  row.names(Control_Spectrums) <- sub("(.*)_(.*)", "\\2_\\1", row.names(
    Control_Spectrums)) #Swaps order around the _
