...
DescriptionGenesis <- function(x) {
  ...
  TheParamList <- map(.x = NewParameters, .f = ParameterDictate,
    data = ParameterAdjust)
  ...
  Lasers <- list()
  if (any(str_detect(Parameters$name, "UV"))) {
    Lasers <- c(Lasers, "UV")
  }
  if (any(str_detect(Parameters$name, "^V"))) {
    Lasers <- c(Lasers, "V")
  }
  if (any(str_detect(Parameters$name, "^B"))) {
    Lasers <- c(Lasers, "B")
  }
  if (any(str_detect(Parameters$name, "^YG"))) {
    Lasers <- c(Lasers, "YG")
  }
  if (any(str_detect(Parameters$name, "^R"))) {
    Lasers <- c(Lasers, "R")
  }
  ...
  LaserData_subset <- LaserData |> filter(LaserOrder %in% Lasers) |>
    mutate(LaserNumber = row_number()) |> relocate(LaserNumber,
      .before = ASF)
  ...
  DisplayList <- map(.x = TheDisplayNames, .f = DisplayInternal,
    data = DisplaySetup)
