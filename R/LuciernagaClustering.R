...
LuciernagaClustering <- function(MyData, These, DetectorName,
  ...
  if (SkipMain == FALSE) {
    ...
    MyData <- MyData |> mutate(Cluster = NULL)
  }

  if (length(These) > 15) {
    stop("Only currently set up to handle up to 16 fluorescence peaks per fluorophore")
    ...
    sixth <- These[[5]]
    seventh <- These[[6]]
  } else if (length(These) == 5) {
    second <- These[[1]]
    third <- These[[2]]
    fourth <- These[[3]]
    fifth <- These[[4]]
    sixth <- These[[5]]
  } else if (length(These) == 4) {
    second <- These[[1]]
    third <- These[[2]]
    fourth <- These[[3]]
    fifth <- These[[4]]
  } else if (length(These) == 3) {
    second <- These[[1]]
    third <- These[[2]]
    fourth <- These[[3]]
  } else if (length(These) == 2) {
    second <- These[[1]]
    third <- These[[2]]
  } else if (length(These) == 1) {
    second <- These[[1]]
  } else if (length(These) == 0) {
    message("No second peak")
  }
  ...
  if (length(These) >= 12) {
    MyData <- MyData |>
      mutate(Cluster = case_when(
        near(MyData[[thirteen]], 0.0) ~
          paste0(MyData$Cluster, thirteen, "_00-"),
        near(MyData[[thirteen]], 0.1) ~
          paste0(MyData$Cluster, thirteen, "_01-"),
        near(MyData[[thirteen]], 0.2) ~
          paste0(MyData$Cluster, thirteen, "_02-"),
        near(MyData[[thirteen]], 0.3) ~
          paste0(MyData$Cluster, thirteen, "_03-"),
        near(MyData[[thirteen]], 0.4) ~
          paste0(MyData$Cluster, thirteen, "_04-"),
        near(MyData[[thirteen]], 0.5) ~
          paste0(MyData$Cluster, thirteen, "_05-"),
        near(MyData[[thirteen]], 0.6) ~
          paste0(MyData$Cluster, thirteen, "_06-"),
        near(MyData[[thirteen]], 0.7) ~
          paste0(MyData$Cluster, thirteen, "_07-"),
        near(MyData[[thirteen]], 0.8) ~
          paste0(MyData$Cluster, thirteen, "_08-"),
        near(MyData[[thirteen]], 0.9) ~
          paste0(MyData$Cluster, thirteen, "_09-"),
        near(MyData[[thirteen]], 1.0) ~
          paste0(MyData$Cluster, thirteen, "_10-")
      ))
  }
  ...
  if (length(These) >= 13) {
    MyData <- MyData |>
      mutate(Cluster = case_when(
        near(MyData[[fourteen]], 0.0) ~
          paste0(MyData$Cluster, fourteen, "_00-"),
        near(MyData[[fourteen]], 0.1) ~
          paste0(MyData$Cluster, fourteen, "_01-"),
        near(MyData[[fourteen]], 0.2) ~
          paste0(MyData$Cluster, fourteen, "_02-"),
        near(MyData[[fourteen]], 0.3) ~
          paste0(MyData$Cluster, fourteen, "_03-"),
        near(MyData[[fourteen]], 0.4) ~
          paste0(MyData$Cluster, fourteen, "_04-"),
        near(MyData[[fourteen]], 0.5) ~
          paste0(MyData$Cluster, fourteen, "_05-"),
        near(MyData[[fourteen]], 0.6) ~
          paste0(MyData$Cluster, fourteen, "_06-"),
        near(MyData[[fourteen]], 0.7) ~
          paste0(MyData$Cluster, fourteen, "_07-"),
        near(MyData[[fourteen]], 0.8) ~
          paste0(MyData$Cluster, fourteen, "_08-"),
        near(MyData[[fourteen]], 0.9) ~
          paste0(MyData$Cluster, fourteen, "_09-"),
        near(MyData[[fourteen]], 1.0) ~
          paste0(MyData$Cluster, fourteen, "_10-")
      ))
  }
  ...
  if (length(These) >= 14) {
    MyData <- MyData |>
      mutate(Cluster = case_when(
        near(MyData[[fifteen]], 0.0) ~
          paste0(MyData$Cluster, fifteen, "_00-"),
        near(MyData[[fifteen]], 0.1) ~
          paste0(MyData$Cluster, fifteen, "_01-"),
        near(MyData[[fifteen]], 0.2) ~
          paste0(MyData$Cluster, fifteen, "_02-"),
        near(MyData[[fifteen]], 0.3) ~
          paste0(MyData$Cluster, fifteen, "_03-"),
        near(MyData[[fifteen]], 0.4) ~
          paste0(MyData$Cluster, fifteen, "_04-"),
        near(MyData[[fifteen]], 0.5) ~
          paste0(MyData$Cluster, fifteen, "_05-"),
        near(MyData[[fifteen]], 0.6) ~
          paste0(MyData$Cluster, fifteen, "_06-"),
        near(MyData[[fifteen]], 0.7) ~
          paste0(MyData$Cluster, fifteen, "_07-"),
        near(MyData[[fifteen]], 0.8) ~
          paste0(MyData$Cluster, fifteen, "_08-"),
        near(MyData[[fifteen]], 0.9) ~
          paste0(MyData$Cluster, fifteen, "_09-"),
        near(MyData[[fifteen]], 1.0) ~
          paste0(MyData$Cluster, fifteen, "_10-")
      ))
  }
  ...
  if (length(These) >= 15) {
    MyData <- MyData |>
      mutate(Cluster = case_when(
        near(MyData[[sixteen]], 0.0) ~
          paste0(MyData$Cluster, sixteen, "_00-"),
        near(MyData[[sixteen]], 0.1) ~
          paste0(MyData$Cluster, sixteen, "_01-"),
        near(MyData[[sixteen]], 0.2) ~
          paste0(MyData$Cluster, sixteen, "_02-"),
        near(MyData[[sixteen]], 0.3) ~
          paste0(MyData$Cluster, sixteen, "_03-"),
        near(MyData[[sixteen]], 0.4) ~
          paste0(MyData$Cluster, sixteen, "_04-"),
        near(MyData[[sixteen]], 0.5) ~
          paste0(MyData$Cluster, sixteen, "_05-"),
        near(MyData[[sixteen]], 0.6) ~
          paste0(MyData$Cluster, sixteen, "_06-"),
        near(MyData[[sixteen]], 0.7) ~
          paste0(MyData$Cluster, sixteen, "_07-"),
        near(MyData[[sixteen]], 0.8) ~
          paste0(MyData$Cluster, sixteen, "_08-"),
        near(MyData[[sixteen]], 0.9) ~
          paste0(MyData$Cluster, sixteen, "_09-"),
        near(MyData[[sixteen]], 1.0) ~
          paste0(MyData$Cluster, sixteen, "_10-")
      ))
  }
