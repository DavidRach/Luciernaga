...
SignatureShifts <- function(data, sampleName = "TheSample") {
  ...
  TheNames <- TheNames[-1]
  AverageReference <- data |>
    filter(.data[[sampleName]] %in% "Average") |>
  ...
  SubtractedData <- OtherNumeric |>
    mutate(across(.cols = everything(),
                  .fns = ~ .x - AverageReference[cur_column()]))

  DriftData <- SubtractedData |>
    mutate(across(everything(), ~ .x / AverageReference[cur_column()]))
