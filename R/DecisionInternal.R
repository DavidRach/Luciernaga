...
#'
#' @importFrom dplyr pull filter bind_rows mutate
#'  case_when arrange desc slice select
#' @importFrom purrr map map2
#'
#' @return An internal value
#'
#' @noRd
DecisionInternal <- function(x, data,
                              FinalConcentration_MillionperML,
                              MillionCellsPerTube, TheConditions,
                              ReturnLeftover) {
  ...
  RestConcentration <- Interest |>
    pull(DesiredConcentration) |> as.double()
  ...
  if (RestConcentration < FinalConcentration) {
    warning(
      "Currently Wetlab_Decisions is only coded going from
  ...
  if (Result >= 1) {
    TheTubes <- map(.x = ConditionsList, .f = ConditionWrap,
    ...
    Tentative <- TheConditions |>
      mutate(MinCells = LowerLimit_MillionCells * CellRatio_Retained *
        1000000) |>
    ...
    if (any(Tentative$Enough == FALSE)) {
      ...
      NewTentative <- NewTentative |>
        mutate(Theoretical = Partition * TotalCells) |>
        mutate(Enough = case_when(Theoretical > MinCells ~ TRUE,
          TRUE ~ FALSE))

      if (any(NewTentative$Enough == FALSE)) {
        NewestTentative <- NewTentative |> arrange(desc(RankImportance)) |>
          slice(-1)
        Residual <- sum(NewestTentative$Partition)
        NewestTentative <- NewestTentative |>
          mutate(Partition = Partition / Residual)
        NewestTentative <- NewestTentative |>
          mutate(Theoretical = Partition * TotalCells) |>
          mutate(Enough = case_when(
            Theoretical > MinCells ~ TRUE, TRUE ~ FALSE))
    ...
    if (Insufficient == FALSE) {
      Sliding <- SlidingScale |>
        select(-c("LowerLimit_MillionCells", "CellRatio_Retained",
          "MinCells", "Enough"))
      ...
      TheTubes <- map2(.x = SliderConditions, .y = SliderCellsPerTube,
        .f = SliderConditionWrap, name = name, Date = Date,
        RestConcentration = RestConcentration,
        FinalConcentration = FinalConcentration,
        TheConditions = TheConditions, TotalCells = TotalCells) |> bind_rows()
