...
#' @importFrom dplyr filter arrange desc select pull rename mutate relocate
...
Luciernaga_SingleColors <- function(x, sample.name, removestrings,
                                     subset, PanelCuts, stats, SignatureView,
                                     Verbose = FALSE, returntype = "data") {
  ...
  if (SignatureView == TRUE) {
    ...
    plot <- ggplot(Melted, aes(x = Detector, y = value, group = Type,
      ...
      scale_color_manual(values = c("Provided" = "blue",
        "Reference" = "red"))
  }
