...
#'
#' @param x  Takes a Levy-Jennings QC tracking report saved as a .csv file,
#'   and converts into a "tidyed" dataframe for plotting. Currently works
#'   on our 3L, 4L, 5L Auroras. Please reach out if you find an issue, the
#'   .csv export varies a bit and I want to continue to improve on the
#'   code to handle these odd exceptions.
#' @param DailyQC A single DailyQCReport .csv file, used to import
#'   baseline settings.
#'
#' @importFrom purrr map2
#' @importFrom tidyr pivot_longer pivot_wider
#' @importFrom tidyselect all_of everything
#' @importFrom dplyr across bind_cols case_when left_join mutate select
...
QC_FilePrep_LJTracking <- function(x, DailyQC) {
  ...
  TheFrames <- map2(.x = StartHere[3], .y = EndHere[3], ReadInfo = ReadInfo,
    .f = ParseThis)
  ...
  GainLonger <- Gains |>
    pivot_longer(all_of(2:GainLength), names_to = "DetectorMain",
      values_to = "Value")
  ...
  Comparison <- Comparison |>
    ...
    select(-Detector, -Comparison) |> mutate(DetectorFlag = paste0("Flag-",
      DetectorMain))
  ...
  RCVsLonger <- RCVs |>
    pivot_longer(all_of(2:RCVsLength), names_to = "RCVMain",
      values_to = "Value")
