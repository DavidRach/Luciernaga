...
#'
#' @param x  Takes a Daily QC CSV file, and converts into a "tidyed"
#'   dataframe for plotting. Currently works on our 3L, 4L, 5L Auroras.
#'
#' @importFrom lubridate ymd hms
#' @importFrom dplyr across case_when mutate relocate rename select
...
QC_FilePrep_DailyQC <- function(x) {
  ...
  if (length(TemperatureIndex) > 0) {
    ...
    Temperature <- data.frame(Temperature = 0, `Flag-Temperature` = FALSE,
      check.names = FALSE)
  }
