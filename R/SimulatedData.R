...
#'
#' @noRd
SimulatedData <- function(populations,
                           abundance,
                           totalevents,
                           targets,
                           distribution,
  ...
  Data <- ReferenceData |>
    select(-Instrument) |>
    filter(Fluorophore %in% TheFluorophores)
  ScaledData <- map(.x = TheFluorophores, .f = FluorScaling, data = Data,
                     targets = targets) |>
    bind_rows()

  IntAbund <- abundance |>
    mutate(Total = Ratio * totalevents) |>
    select(-Ratio)
  ToAssemble <- left_join(IntAbund, populations, by = "Pops")

  Pops <- ToAssemble |> pull(Pops)

  ThePopulations <- map(.x = Pops, .f = DataSimulation, ToAssemble = ToAssemble,
                         ScaledData = ScaledData, distribution = distribution)

  Dataset <- do.call(rbind, ThePopulations)

  Dataset <- data.frame(Dataset)
  Dataset <- Dataset |>
    mutate(DateTime = "Standin", Fluorophore = "Standin") |>
  ...
  files <- list.files(path = path, pattern = "CD4_BUV805.*Cells",
                       full.names = TRUE)
  CytoSet <- load_cytoset_from_fcs(files, truncate_max_range = FALSE,
