...
#'
#' @return A data.frame containing the combined structured output.
#'
#' @noRd
TwoLineSandwhich <- function(x, data) {
  ...
  SecondData <- as.data.frame(
    as.list(setNames(vals, keys)), check.names = FALSE
  )
  colnames(SecondData) <- paste0(
    colnames(FirstData[2]), " ", colnames(SecondData)
  )
