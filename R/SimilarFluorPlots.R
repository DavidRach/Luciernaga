...
#'
#' @noRd
SimilarFluorPlots <- function(TheseFluorophores,
                               TheFluorophore,
                               data,
                               legend = TRUE,
                               plotname = FALSE,
  ...
  if (!is.null(unstained)) {
    ...
    UnstainedData <- unstained |>
      ...
      pivot_longer(cols = everything(), names_to = "Detector",
                   values_to = "value") |>
  ...
  if (!is.null(plotlinecolor)) {
    ThePlot <- ggplot(TheData, aes(x = Detector, y = value,
      ...
      scale_x_discrete(breaks = unique(TheData$Detector)[c(TRUE,
                                                             rep(FALSE, 4))])
  } else {
    ThePlot <- ggplot(TheData, aes(x = Detector, y = value,
                                    group = Fluorophore,
