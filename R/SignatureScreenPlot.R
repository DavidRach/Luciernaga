...
#' 
#' @noRd
SignatureScreenPlot <- function(data,
                                 RetainThese = NULL,
                                 normalize = FALSE,
                                 legend = FALSE,
                                 linecolor = "red",
  ...
  Values <- Filtering |>
    filter(ClusterStart %in% MainValue) |>
    select(-Count, -Ratio) |>
    select(where(is.numeric))
  Average <- AveragedSignature(x = Values, stats = median)
  Average <- Average |>
    mutate(TheSample = "Average") |>
  ...
  Filtering1 <- Filtering1 |>
    ...
    select(-Count, -Ratio) |>
    select(TheSample, where(is.numeric))
  ...
  Melted[["TheSample"]] <- factor(
    Melted[["TheSample"]],
    levels = c(setdiff(unique(Melted[["TheSample"]]), "Average"), "Average")
  )
