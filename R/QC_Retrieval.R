#' Returns Detector Gains, Laser Delay and Scalings from individual .fcs
#' files in a CytoSet.
#'
...
QC_Retrieval <- function(x, sample.name) {
  ...
  ParameterRows <- map2(.x = PN_Names, .y = PV_Gains,
    .f = Luciernaga:::RetrievalMerge, TheData = KeywordsDF) |> bind_cols()
  ...
  LaserASFRows <- map2(.x = Laser_Name, .y = Laser_ASF,
    .f = Luciernaga:::RetrievalMerge, TheData = KeywordsDF) |> bind_cols()
