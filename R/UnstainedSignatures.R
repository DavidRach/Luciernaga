...
#' @param alternatename Alternate name descriptor
...
#'
#' @importFrom dplyr filter select mutate across relocate arrange left_join
#'   pull case_when near desc
...
UnstainedSignatures <- function(x, WorkAround1, alternatename, ColsN,
                                ...
                                EndNormalizedMergedCol, Increments,
                                Verbose = FALSE, LocalMaximaRatio = 0.15,
                                SecondaryPeaks) {
  ...
  # DetectorPeakCounts(
  #   x = MySubset,
  #   StartN = StartNormalizedMergedCol,
  ...
  Decoys <- Decoys |>
    mutate(TheDetector = 1:nrow(Decoys)) |>
  ...
  PointData <- LocalMaxima(
    theX = LocalX, theY = LocalY, therepeats = 3, w = 3,
  ...
  MyData <- LuciernagaClustering(
    MyData = MyData, These = These, DetectorName = DetectorName
  )
