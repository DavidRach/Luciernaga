...
#' @param Subtraction Dictates what autofluorescence gets extracted. Options include
#' "Internal", "Average"
...
#'
#' @importFrom dplyr filter arrange select mutate relocate bind_rows slice
#'  pull left_join across
#' @importFrom tidyselect all_of where
#' @importFrom stringr str_starts
#' @importFrom ggplot2 ggplot geom_bar theme_bw theme element_text labs
...
#'
#' @noRd
SingleStainSignatures <- function(x,
                                   WorkAround1,
                                   ColsN,
  ...
  if (Subtraction == "Internal") {
    ...
    AutofluorescentSubset <- WorkAround1 |>
      filter(.data[[TheMainAF]] %in% 1.000) |>
      arrange(desc(.data[[x]]))
    # nrow(AutofluorescentSubset)
    # DetectorPeakCounts(x = AutofluorescentSubset, StartN = 65, EndN = 128)

    SingleColorSubset <- WorkAround1 |>
      dplyr::filter(.data[[x]] %in% 1.000) |>
    ...
    AutofluorescentSubsetB <- AutofluorescentSubset |>
      select(all_of((StartNormalizedMergedCol + 1):
                       (EndNormalizedMergedCol + 1))) |>
      mutate(across(where(is.numeric),
                     ~ ceiling(. / Increments) * Increments)) |>
      mutate(Backups = AutofluorescentSubset$Backups) |>
      relocate(Backups, .before = 1)

    SingleColorSubsetB <- SingleColorSubset |>
      select(all_of((StartNormalizedMergedCol + 1):
                       (EndNormalizedMergedCol + 1))) |>
      mutate(across(where(is.numeric),
                     ~ ceiling(. / Increments) * Increments)) |>
      mutate(Backups = SingleColorSubset$Backups) |>
      relocate(Backups, .before = 1)
    ...
    SingleColorNamed <- SignatureCluster(Arg1 = x, Arg2 = TheMainAF,
                                          data = SingleColorSubsetB)
    SignatureData <- bind_rows(AutofluorescenceNamed, SingleColorNamed)
    TheClusters <- data.frame(table(SignatureData$Cluster),
                               check.names = FALSE)
    colnames(TheClusters)[1] <- "Clusters"
    colnames(TheClusters)[2] <- "Counts"

    AF <- TheClusters |>
      filter(str_starts(Clusters, TheMainAF)) |>
      arrange(Clusters)
    SC <- TheClusters |>
      filter(str_starts(Clusters, x)) |>
    ...
    AFforSubtraction <- Total |>
      filter(str_starts(Clusters, TheMainAF)) |>
      arrange(desc(Counts)) |>
      slice(1) |>
      pull(Clusters) # |> as.character()
  }


  if (Subtraction == "Internal") {

    AF_Choice <- SignatureData |>
      filter(Cluster %in% AFforSubtraction) |>
      select(Backups)
    ...
    # Samples: Either Original, desiredAF driven alternative, or external
    # provided.
    StoredBackups <- WorkAround1 |> select(Backups)
    Data <- WorkAround1 |> select(-Backups)
    Data <- Data |> select(all_of(1:ColsN))
  ...
  if (Verbose == TRUE) {
    ...
    message(round(BelowZero / TheTotal, 2),
            " of all events post AF subtraction were negative and will be",
            " rounded to 0")
  }
  ...
  RevisedPeakDetectorCounts <- data.frame(Fluors = names(Counts2),
                                           Counts = Counts2)
  rownames(RevisedPeakDetectorCounts) <- NULL
  RevisedPeakDetectorCounts <- RevisedPeakDetectorCounts |>
    arrange(desc(Counts))
  ...
  TheRevisedList <- RevisedPeakDetectorCounts |>
    filter(Counts > 0) |>
    pull(Fluors)

  if (!x %in% TheRevisedList) {
    return(NULL)
  }
  ...
  if (SCData == "subtracted") {
    WorkAround2 <- cbind(StoredBackups, Test, Normalized2)
  }
  if (SCData == "raw") {
    WorkAround2 <- cbind(StoredBackups, Data, Normalized2)
  }
  ...
  SingleColorSubset2 <- WorkAround2 |>
    dplyr::filter(.data[[x]] %in% 1.000) |>
    arrange(desc(.data[[TheMainAF]]))
  # nrow(SingleColorSubset2)
  # DetectorPeakCounts(x = SingleColorSubset2, StartN = 65, EndN = 128)

  SingleColorSubset2B <- SingleColorSubset2 |>
    select(all_of((StartNormalizedMergedCol + 1):
                     (EndNormalizedMergedCol + 1))) |>
    mutate(across(where(is.numeric),
                   ~ ceiling(. / Increments) * Increments)) |>
    mutate(Backups = SingleColorSubset2$Backups) |>
    relocate(Backups, .before = 1)

  SingleColorData <- SignatureCluster(Arg1 = x, Arg2 = TheMainAF,
                                       data = SingleColorSubset2B)
  TheClusters <- data.frame(table(SingleColorData$Cluster),
                             check.names = FALSE)
  colnames(TheClusters)[1] <- "Clusters"
  colnames(TheClusters)[2] <- "Counts"
  Total <- TheClusters |>
  ...
  FinalPlot <- ggplot(Total, aes(x = Clusters, y = Counts)) +
    ...
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(title = paste(x, AggregateName, "Final"), sep = " ")
  ...
  MainClusters <- TheClusters |>
    filter(Counts >= Cutoff) |>
    select(Clusters) |>
    pull() |>
    as.character()
  ...
  Ready <- Ready |>
    select(-all_of((StartNormalizedMergedCol + 2):
                      (EndNormalizedMergedCol + 2)))
  ...
  AllData <- map(.x = MainClusters, .f = ClusterIteration, data = TheReady,
                 ...
                 Verbose = Verbose, LocalMaximaRatio = LocalMaximaRatio,
