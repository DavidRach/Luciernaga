#' Highlight the location of a particular cell population on a given
#' bi-exponential axis.
#'
...
#' @param tilesize Equivalent of bin, determines the height and width of the
#'  tile dots.
#' @param reference Provide Coereba reference when splitpoint equals
#'  crossreference
#' @param FactorNames Provide factor names when splitpoint equals categorical
#'
#' @importFrom flowWorkspace gs_pop_get_data
#' @importFrom flowCore exprs
#' @importFrom dplyr mutate row_number pull filter select
#' @importFrom ggplot2 ggplot geom_tile aes scale_fill_gradient theme_bw labs
...
#'
Utility_ThirdColorPlots <- function(x,
                                     subset,
                                     xaxis,
                                     yaxis,
                                     zaxis,
  ...
  if (is.data.frame(splitpoint)) {
    message("Splitpoint is a Dataframe")
    ...
    for (i in FilterArguments) {
      ...
      FilteringDF <- FilteringDF |>
        dplyr::filter(.data[[zaxis]] >= TheSplit)
    }

    if (!nrow(FilteringDF) > 0) {
      stop("Nothing left after filtering arguments")
    }

    RetainedIDs <- FilteringDF |> select(NewID) |> pull()
    TheSubset <- FilteringDF |> select(-NewID)
    TheBackground <- TheDF |>
      filter(!NewID %in% RetainedIDs) |>
      select(-NewID)

    Plot <- ggplot() +
      geom_tile(data = TheBackground,
                aes(x = .data[[xaxis]], y = .data[[yaxis]]),
                width = tilesize, height = tilesize, color = "lightgray",
                fill = "lightgray") +
      geom_tile(data = TheSubset,
                aes(x = .data[[xaxis]], y = .data[[yaxis]]),
                width = tilesize, height = tilesize, color = thecolor,
                fill = thecolor) +
  ...
  } else if (splitpoint == "continuous") {
    message("Splitpoint is a continuous")

    Plot <- ggplot(TheDF) +
      geom_tile(aes(x = .data[[xaxis]], y = .data[[yaxis]],
                    fill = .data[[zaxis]]), width = tilesize,
                height = tilesize, color = "lightgray") +
      scale_fill_gradient(low = "lightgray", high = thecolor) +
      theme_bw() +
      labs(title = AggregateName) +
      theme(strip.background = element_blank(),
  ...
  } else if (is.numeric(splitpoint)) {
    splitpoint <- splitpoint
  } else if (is.character(splitpoint)) {
    splitpoint <- as.numeric(splitpoint)
  } else {
    stop("splitpoint argument not recognized")
  }
