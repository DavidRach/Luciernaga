...
#'   ggplot labs scale_x_discrete theme theme_bw
#'
...
QC_RefPlots <- function(x, Data, references = FALSE, refData = NULL) {
  if (!references == TRUE) {
    ...
    ThePlot <- ggplot(TheValue, aes(x = Detector, y = value,
      group = Sample)) +
      geom_line() + theme_bw() +
      labs(title = paste0(TheFluorochrome, " ", x), x = NULL,
        y = "Normalized Value") +
    ...
    if (any(ReferenceFluorList == TheFluorochrome)) { #TheCleanedVersion
      ...
      ReferenceData1$Detector <- factor(ReferenceData1$Detector,
        levels = MyVector)
      ...
      ThePlot <- ggplot(TheValue, aes(x = Detector, y = value,
        group = TheSamples)) +
        geom_line() + theme_bw() +
        labs(title = paste0(x), x = NULL, y = "Normalized Value") +
        geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
        theme(plot.title = element_text(size = 8),
          axis.text.x = element_text(size = 6, angle = 45),
      ...
      ThePlot <- ThePlot + geom_line(data = ReferenceData1,
        aes(x = Detector, y = value, group = TheSamples), color = "red")
    } else {
      TheValue <- Data |> filter(TheSamples %in% x)
      TheFluorochrome <- TheValue |> select(Fluorochrome) |> unique() |>
        pull()
      ThePlot <- ggplot(TheValue, aes(x = Detector, y = value,
        group = Sample)) +
        geom_line() +
