...
QCLibraryParse <- function(x, returntype, references=TRUE){
  ...
  TheValue2 <- TheValue %>% mutate(Detector=1:nrow(.)) %>% relocate(Detector, .before=TheValue)

  if (returntype == "dataframe"){

    Assembling <- TheValue2 %>% pivot_wider(names_from = Detector, values_from = TheValue)
    ...
    if(references==TRUE){
      ...
      if (any(ReferenceFluorList == Fluorochrome)){ #TheRegularVersion
        ...
        ThePlot <- ggplot(TheValue2, aes(x=Detector, y=TheValue)) + geom_line() + theme_bw() +
          labs(title=paste0(Fluorochrome, " ", Sample), y="Normalized") +
          geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
          theme(plot.title = element_text(size = 8), axis.title.y =  element_text(size=8))

        ThePlot <- ThePlot + geom_line(data = ReferenceData1, aes(x=Detector, y=TheValue), color="red")

      } else {
        Fluorochrome <- gsub("AF", "Alexa Fluor", gsub("efl", "eFl", gsub("Spk", "Spark", Fluorochrome)))
        Fluorochrome <- gsub(" ", "", gsub("-", "", gsub(".", "", fixed=TRUE, Fluorochrome)))
        ...
        if (any(ReferenceFluorList == Fluorochrome)){ #TheCleanedVersion
          ...
          ThePlot <- ggplot(TheValue2, aes(x=Detector, y=TheValue)) + geom_line() + theme_bw() +
            labs(title=paste0(Fluorochrome1, " ", Sample), y="Normalized") +
            geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
            theme(plot.title = element_text(size = 8), axis.title.y =  element_text(size=8))
