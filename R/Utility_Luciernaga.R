...
#' @param LuciernagaVariables  The data.frame column that contains your
#' flourophores (ex. "Samples" -> "APCFire750")
#' @param LuciernagaClusters The data.frame column that contains your
#' clusters (ex. "Clusters" -> "R01_10-R02_10-R03_10")
...
Utility_Luciernaga <- function(LuciernagaData, LuciernagaVariables,
                               LuciernagaClusters, outfolder, filename){
  Items <- data.frame(table(LuciernagaData[[LuciernagaVariables]])) %>%
  ...
  for (i in Items){LuciernagaSubset <- LuciernagaData %>%
    filter(.[[LuciernagaVariables]] %in% c(i))
  colnames(LuciernagaSubset) <- gsub("-A", "", colnames(LuciernagaSubset))
  zero_sum_rows <- LuciernagaSubset %>% filter(rowSums(
    select(., all_of(FirstDetectorColumn:LastDetectorColumn)),
    na.rm = TRUE) == 0)
  zero_sums <- nrow(zero_sum_rows)
  print(paste0(i, sep = "  ", zero_sums))
  DemonSpawn <- LuciernagaSubset %>% filter(rowSums(select(
    ., all_of(FirstDetectorColumn:LastDetectorColumn)), na.rm = TRUE) == 0)
  LuciernagaSubset <- LuciernagaSubset %>% filter(rowSums(select(
    ., all_of(FirstDetectorColumn:LastDetectorColumn)), na.rm = TRUE) != 0)
  #Temporary Debugging WorkAround.

  LinePlotData <- LuciernagaSubset %>% select({{LuciernagaClusters}}, {{
    FirstDetectorColumn}}:{{LastDetectorColumn}})
  LineCols <- ncol(LinePlotData)
  detector_order <- colnames(LuciernagaSubset)[
    FirstDetectorColumn:LastDetectorColumn]

  Melted <- gather(LinePlotData, key = "Detector", value = "value",
  ...
  TheNormPlot <- ggplot(Melted1, aes(x = Detector, y = value,
    group = Cluster, color = Cluster)) + geom_line() +
    ylim(min = Low, max = High) +
    labs(title = "Fluorophores", x = "Detectors", y = "Normalized Values") +
    ...
    theme(plot.title = element_text(size = 16L, face = "plain", hjust = 0.5),
          ...
          linetype = "twodash"), panel.grid.minor = element_line(
            colour = "gray95",linetype = "longdash"),
...
Utility_SCPatchwork(x = theflattenedplots, filename = filename,
                    outfolder = outfolder)
}
