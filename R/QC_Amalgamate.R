#' Taking a Luciernaga_Slices (or similar data.frame) with an added Counts
#' column, expands each row by the count, takes the median, and then
#' returns either data or a plot showing how different the average
#' signature is from the individual ones
#'
...
#' @param linecolor Default is red, specify alternate color for the
#'  Average signature
...
QC_Amalgamate <- function(data, samplecolumn = "Percentiles",
                           normalize = FALSE, countcolumn = "Count",
                           returnType = "plot", titlename = NULL,
                           linecolor = "red", legend = TRUE) {
  Combined <- data |>
    tidyr::uncount(weights = !!sym(countcolumn), .remove = TRUE) |>
    select(-all_of(samplecolumn)) |> summarize(across(everything(), median)) |>
    mutate(!!samplecolumn := "Average") |> relocate(all_of(samplecolumn),
      .before = 1)
  ...
  if (returnType == "data") {
    ...
    Melted[[samplecolumn]] <- factor(Melted[[samplecolumn]],
      levels = c(setdiff(unique(Melted[[samplecolumn]]), "Average"),
        "Average"))
    ...
    plot <- ggplot(Melted, aes(x = Detector, y = value,
      group = .data[[samplecolumn]], color = .data[[samplecolumn]])) +
      geom_line() +
      scale_color_manual(values = setNames(
        ifelse(unique(Melted[[samplecolumn]]) == "Average", linecolor,
          "gray"),
