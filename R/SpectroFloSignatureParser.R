...
#'
#' @importFrom xml2 read_xml xml_children xml_name xml_text
...
#' @noRd
SpectroFloSignatureParser <- function(x,
                                       ColumnNames = "detector",
                                       returnType = "data") {
  Parsed <- read_xml(x)
  ...
  if (any(xml_name(Landing) == "ExperimentData")) {
    ...
    if (length(Experiment_child) == 0) {
      ...
      if (any(xml_name(Info_child) == "ExperimentDesc")) {
        ExperimentDesc <- Info_child[
          xml_name(Info_child) == "ExperimentDesc"][[1]]
        Experiment_child <- xml_children(ExperimentDesc)
      } else {
        message("Missed version for ", x)
      }
    }
  ...
  RefSetUp <- Experiment_child[
    xml_name(Experiment_child) == "_RefSetupResult"][[1]]
  RefSetUp_child <- xml_children(RefSetUp)

  if (length(RefSetUp_child) != 0) {
    SpillOverColumn <- RefSetUp_child[
      xml_name(RefSetUp_child) == "SpilloverColumnList"][[1]]
    Spill_child <- xml_children(SpillOverColumn) # Number Children
    Data <- map(.x = Spill_child, .f = Luciernaga:::NormalizedParser) |>
      bind_rows()
    ...
    message(paste("The .Expt lacked Unmixing parameters (was Raw),",
                   "returning a NULL value instead of intended",
                   "data.frame row"))
    Value <- NULL
    return(Value)
  }
