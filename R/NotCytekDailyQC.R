...
#'
#' @param x  A data.frame or file.path to the .csv with the Detector,
#'   GainBaseline and RCVCutoff
#'
...
NotCytekDailyQC <- function(x) {
  ...
  if (any(!TheseColumns %in% colnames(Data))) {
    stop("CSV file should contain `Detector`, `GainBaseline` and ",
      "`RCVCutoff` as column names")
  }
