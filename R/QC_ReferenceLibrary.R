#' Queries the available reference library for available fluorophores (and
#' their naming conventions)
#'
#' @param FluorNameContains A character string pattern to match, example
#'   "APC"
...
#'   For ThermoFisher BigFoot 7L_488-561=55, 7L_532-594="52_7L",
#'   6L_445="52_6L", 6L_785=51
#' @param returnPlots Whether to return signature plot as well. Default
#'   FALSE.
...
#' @param unstained Default NULL, alternatively provide corresponding
#'   unstained signature
#'
#' @importFrom purrr map
#' @importFrom dplyr arrange bind_rows desc filter group_by mutate n pull
#'   select slice ungroup
#' @importFrom stringr str_detect str_extract
#'
#' @return A dataframe column containing matching Fluorophores from your
#'   querry
#' @export
#'
#' @examples
...
QC_ReferenceLibrary <- function(FluorNameContains, NumberDetectors,
                                 ...
                                 exact = FALSE, unstained = NULL) {
  if (!length(NumberDetectors) == 1) {
    ReferenceData <- map(.x = NumberDetectors,
      .f = Luciernaga:::InstrumentReferences) |> bind_rows()
  ...
  if (exact == FALSE) {
    if (length(FluorNameContains) == 1) {
      ...
      Subset <- TheList |>
        filter(str_detect(Fluorophore,
          paste(FluorNameContains, collapse = "|")))
    }
  } else {
    if (length(FluorNameContains) == 1) {
      Subset <- TheList |> filter(str_detect(Fluorophore, FluorNameContains))
  ...
  if (returnPlots == FALSE) {
    ...
    if (!length(NumberDetectors) == 1) {
      ...
      ThePlot <- Luciernaga:::SimilarFluorPlots(
        TheseFluorophores = TheseFluorophores, TheFluorophore = NULL,
        data = ReferenceData, plotlinecolor = plotlinecolor, legend = legend,
