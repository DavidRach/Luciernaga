...
#'   path to the folder containing the respective .fcs files
#'   contained within that workspace
...
CellPopExtract <- function(x, path, keywords="GROUPNAME", subset,
 outpath, addon=NULL, downsample=NULL, metadataOverride=NULL) {
  ...
  if (inherits(x, "GatingSet")) {
    ...
    ws <- open_flowjo_xml(x)
    gs <- flowjo_to_gatingset(ws, name=1, path = path,
         keywords = keywords)
  }

  if (!is.null(downsample)) {
    walk(.x=gs, .f=Utility_Downsample,
         sample.name = keywords,
    subsets = subset, subsample = downsample, internal = FALSE,
     export = TRUE,
    inverse.transform=TRUE, outpath = outpath, addon=addon)
  } else {
    SubsetSolo <- gs_pop_get_data(gs, subset,
         inverse.transform = TRUE)
    walk(.x=SubsetSolo, .f=FCS_Subset_Copy,
