#' Takes Before After QC .fcs files of QC beads run on assay settings,
#' returns the gains and average measurement in a data.frame to allow for
#' plotting to assess stability over time.
#'
#' @param x A CytoSet (when mapped) or an individual cytoset, example
#'   (x=MyCytoSet[[1]])
#' @param sample.name The keyword value that distinguishes individual .fcs
#'   files
#' @param stats Whether to summarize the MFIs by "mean" or "median"
#' @param subsets When provided with a GatingSet, selects this subset to
#'   provide the Cytoset
...
QC_GainMonitoring <- function(x, sample.name, stats, subsets = NULL,
  ...
  if (class(x) == "GatingHierarchy") {
    ...
    if (nrow(cs[[1]]) != 0) {
      ...
      message("No cells retained in ", SayTheName,
        ", passing original .fcs file")
  ...
  if (str_detect(Bound[["SAMPLE"]], "efore")) {
    Bound <- Bound |> mutate(Timepoint = "Before") |>
      relocate(Timepoint, .after = TIME)
  } else if (str_detect(Bound[["SAMPLE"]], "fter")) {
    Bound <- Bound |> mutate(Timepoint = "After") |>
      relocate(Timepoint, .after = TIME)
  } else {
    Bound <- Bound |> mutate(Timepoint = "Unknown") |>
      relocate(Timepoint, .after = TIME)
  }
  return(Bound)
}