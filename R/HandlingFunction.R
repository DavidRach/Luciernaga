...
HandlingFunction <- function(x) {
  ...
  CellsUnstained <- Unstained[!str_detect(Unstained, "Beads|Dead")]

  Stained <- files[!str_detect(files, "Unstained")]
  BeadsStained <- Stained[grep("Beads", Stained)]
  CellsStained <- Stained[!str_detect(Stained, "Beads")]

  # References
  FileLocation <- system.file("extdata", package = "Luciernaga")
  CellGates <- fread(file.path(path = FileLocation, pattern = "Gates.csv"))
  CellTemplate <- gatingTemplate(CellGates)
  BeadGates <- fread(file.path(path = FileLocation,
  ...
  These <- c(UnstainedCellPlot, StainedCellPlot, UnstainedBeadPlot,
             StainedBeadPlot)

  OnLocation <- list.dirs(x, full.names = TRUE, recursive = TRUE)
  Presence <- list.files(OnLocation, pattern = "Luciernaga",
                          full.names = TRUE)
  if (length(Presence) == 0) {
    dir.create(file.path(x, "Luciernaga"))
  }
  Presence <- list.files(OnLocation, pattern = "Luciernaga",
                          full.names = TRUE)
  ...
  UnstainedCellAF <- Luciernaga_QC(x = UnstainedCell_GS[[1]],
                                    ...
                                    experiment = "SingleColor",
                                    condition = "Cells")

  Unstained <- map(.f = Luciernaga_QC, .x = UnstainedCell_GS,
                    subsets = "nonDebris",
                    removestrings = ".fcs",
                    sample.name = "GUID",
                    unmixingcontroltype = "cells",
  ...
  Colors <- map(.f = Luciernaga_QC, .x = StainedCell_GS,
                subsets = "nonDebris",
                removestrings = ".fcs",
                sample.name = "GUID",
                unmixingcontroltype = "cells",
