...
#' @param AggregateName Passed final name with modifications from
#'  name
#' @param outpath Location to export the fcs and .csv files to
#' @param OriginalStart Passed Argument indicating start column for
#'  Raw .fcs values
#' @param OriginalEnd Passed argument indicating end column for raw
#'  .fcs values
...
#' @param TotalNegatives How many of the above rows to append,
#'  default is set to 500
#' @param Samples When Negative type = "Internal", the data.frame
#'  of averaged
...
#'
#' @importFrom dplyr filter select mutate across bind_cols relocate
...
#'
#' @noRd
InternalGenesis <- function(x, Data, AggregateName, outpath=NULL,
   OriginalStart, OriginalEnd, stats="median",
   NegativeType="default", TotalNegatives = 500,
  ...
  if (NegativeType == "artificial"){
    ...
    mutateCols <- MeanFCS[,-grep("Time|FS|SC|SS|Original|W$|H$",
     names(MeanFCS))] %>%
      colnames(.)
    MeanFCS <- MeanFCS %>% mutate(across(all_of(mutateCols),
     ~ifelse(. >= 0, 0, .)))
  ...
  if (NegativeType == "samples"){
    if(!is.data.frame(Samples)){stop(
      "Samples needs to be a single row of a data.frame for
       just the raw detectors")}
