...
RestInternal <- function(x,
  ...
  CurrentConcentration <- Internal |>
    select(ConcentrationScientific) |>
    pull()
  ...
  Instructions <- cbind(Name, Date, CurrentConcentration, TotalVolume,
                         TotalCells, DesiredConcentration, NeededVolume,
                         IncreaseVolumeML, TubeMaxML, TotalTubes)
  ...
  if (Instructions$SpinDown == TRUE) {
    ...
    SpinProtocol <- SpinProtocol |>
      mutate(TotalVolume = case_when(
        TotalVolume > 0 ~ NA_real_,
        TRUE ~ TotalVolume
      ))
    SpinProtocol <- SpinProtocol |>
      mutate(CurrentConcentration = case_when(
        CurrentConcentration > 0 ~ NA_real_,
        TRUE ~ CurrentConcentration
      ))
    ...
    A <- TotalCells / (DesiredConcentration_MillionperML * 1000000)
    SpinProtocol <- SpinProtocol |>
      mutate(NeededVolume = case_when(
        NeededVolume > 0 ~ A,
        TRUE ~ NeededVolume
      ))
    SpinProtocol <- SpinProtocol |>
      mutate(IncreaseVolumeML = case_when(
        is.double(IncreaseVolumeML) ~ A,
        TRUE ~ IncreaseVolumeML
      ))
    if (SpinProtocol$IncreaseVolumeML > 0) {
      SpinProtocol <- SpinProtocol |>
        mutate(SpinDown = case_when(
          is.logical(SpinDown) ~ FALSE,
          TRUE ~ SpinDown
        ))
    }
    TotalTubesTwo <- A / TubeMaxML
    SpinProtocol <- SpinProtocol |>
      mutate(TotalTubes = case_when(
        is.double(TotalTubes) ~ TotalTubesTwo,
        TRUE ~ TotalTubes
      ))
    NewName <- SpinProtocol |> pull(name) %>% paste0("Spin_", .) # TODO: `.` in 2nd position (magrittr-only)
    SpinProtocol <- SpinProtocol |>
      mutate(name = case_when(
        is.character(name) ~ NewName,
        TRUE ~ name
      ))
    SpinProtocol$NeededVolume <- format(SpinProtocol$NeededVolume, digits = 2)
    SpinProtocol$IncreaseVolumeML <- format(SpinProtocol$IncreaseVolumeML,
                                             digits = 2)
