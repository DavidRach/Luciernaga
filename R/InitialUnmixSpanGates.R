...
#' @examples A <- 2 + 2
#' 
InitialUnmixSpanGates <- function(template,
                                   gs,
                                   subset,
                                   minpercentile = 0.5,
                                   maxpercentile = 0.99,
  ...
  walk(.x = GatesToAdd, .f = InitialSpans, gs = gs, subset = subset,
       data = DetectorsPresent, inverse.transform = inverse.transform,
       minpercentile = minpercentile, maxpercentile = maxpercentile,
       .progress = TRUE)
}