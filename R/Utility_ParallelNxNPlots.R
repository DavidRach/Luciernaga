...
#'
Utility_ParallelNbyNPlots <- function(x,
                                       y,
                                       sample.name,
                                       removestrings,
                                       Override = FALSE,
  ...
  if (AltNameX == AltNameY) {
    message("X and Y names match, attaching numbers")
  ...
  # Would need to modify NameFor to handle multiple names if incorporating
  # here.
  ...
  if (all(X_DFNames == Y_DFNames)) {
    DFNames <- X_DFNames
    PlotNumber <- X_PlotNumber
  } else {
    stop("The two fcs files do not have matching column parameters")
  }
  ...
  if (ycolumn == "ALL") {
    ...
    Plots <- map(.x = DFNames, .f = ParallelUniversalIterator, x_ff = x_ff,
                 y_ff = y_ff, TheDF = TheDF, yValue = ycolumn,
                 columnlist = DFNames, gatelines = gatelines,
                 reference = reference, clearance = clearance, bins = bins,
                 AltNameX = AltNameX, AltNameY = AltNameY, colorX = colorX,
                 colorY = colorY)
  } else {
