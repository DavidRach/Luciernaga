...
#' @param historydata A list of LevyJenningParse updated data objects
#' @param timewindow The number  desired months
#' 
#' @importFrom purrr map
#' @importFrom dplyr bind_rows group_by mutate filter pull slice ungroup n
...
QCHistoryArchive <- function(x, historydata, timewindow = 24) {
  ...
  TheDataset <- map(.x = x, data = historydata, .f = InternalColorCodeStatus) |>
    bind_rows()
