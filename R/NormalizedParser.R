...
#'
#' @importFrom xml2 xml_children xml_name xml_text xml_find_all xml_ns
...
NormalizedParser <- function(x) {
  ...
  FluorophoreFloat <- RefControl_child[
    xml_name(RefControl_child) == "Fluorochrome"]
  ...
  FloatingValues <- xml_find_all(Param_child, ".//d7p1:float",
    ns = xml_ns(Param_child))

  if (length(FloatingValues) == 0) {
    FloatingValues <- xml_find_all(Param_child, ".//d6p1:float",
      ns = xml_ns(Param_child))
  }
