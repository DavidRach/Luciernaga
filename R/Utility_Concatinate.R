...
#' @param subsample Default is set to NULL. If down-sample is desired, Total
#'   number of events to subsample from each specimen
#' @param ReturnType Whether to return as a "data.frame", a "flow.frame", or
#'   export to an "fcs" file to the outpath location
...
#' @param inverse.transform Whether to reverse the GatingSet Transform on the
#'   data, default is set to FALSE.
#' @param metadataCols column names from pData to append as metadata for the
#'   .fcs, default NULL
...
#' @importFrom flowCore write.FCS parameters exprs
#'
#' @return A concatenated data.frame, flow.frame or fcs file, with reference
#'   .csv for the specimen information
#' @export
#'
#' @examples
#'
...
#'
#' ConcatinatedReturn <- Utility_Concatinate(
#'   gs = UnmixedGatingSet,
...
#' )
#'
Utility_Concatinate <- function(gs, sample.name, removestrings, subsets,
                                subsample = NULL, ReturnType, newName,
                                outpath = NULL, export = FALSE,
                                inverse.transform, metadataCols = NULL,
  ...
  if (Override == FALSE) {
    ...
    if (!is.null(subsample)) {
      if (!is.numeric(subsample)) {
        stop(
          "Subsample argument is not numeric, enter just a number, no quotes needed"
        )
      }
    }

    ConcatenatedFile <- map(
      gs,
      .f = Utility_Downsample, sample.name = sample.name,
      removestrings = removestrings, subsets = subsets,
      subsample = subsample, internal = TRUE, export = FALSE,
      metadataCols = metadataCols, inverse.transform = inverse.transform
    ) |>
      bind_rows()
    ...
    ConcatenatedFile <- DataOverride
  }

  if (is.null(metadataCols)) {
    metadataCols <- setdiff(
      colnames(ConcatenatedFile),
      colnames(gs[1])
    )
  }
  ...
  for (i in metadataCols) {
    ConcatenatedFile <- ExecuteNumerics(
      x = i, data = ConcatenatedFile,
  ...
  if (!nrow(Decisions) == 0) {
    CharResults <- map(
      .x = metadataCols, .f = ExecuteCharacters,
      data = ConcatenatedFile, conversion = Decisions
    )
    CharResults <- Filter(Negate(is.null), CharResults)

    if (length(CharResults) > 1) {
      Table <- bind_cols(CharResults)
    } else {
      Table <- CharResults[[1]]
    }
    ...
    if (export == TRUE) {
      write.csv(x = Table, file = CSVDestination, row.names = FALSE)
    }
  ...
  new_fcs <- Utility_ColAppend(
    ff = ff, DF = ConcatenatedMain,
    columnframe = ConcatenatedExtra
  )
