...
#' @param filterDate Used to quickly return a screened plot, use the plot
#'   title name in it's entirety and set returnType to "plot"
...
#' @importFrom dplyr filter group_by mutate pull ungroup
...
QC_Screen <- function(data, groupColumns, clusterColumn = "Cluster",
                       filterDate = NULL, cutoff = 0.005, normalize = FALSE,
                       returnType = "pdf", outpath,
                       filename = "BeadQCVisualized", therows = 3,
                       thecolumns = 3) {
