...
Utility_Phate <- function(x, sample.name, removestrings, subsets, columns, notcolumns, subsample, export, outpath){
  #Python Environment Startup # Early in Case of Failure
  proc <- basiliskStart(env2)
  ...
  new_fcs <- Utility_ColAppend(ff=newff, DF=DF, columnframe=ThePhate)
