...
#' "normalized" values
#' @param TheSummary Whether to return individual cells or
#'  summarized by stats.
#' @param stats TBD
#'
#' @importFrom dplyr filter pull mutate across select group_by
#'  ungroup arrange left_join bind_rows desc
...
#'
#' @noRd
FCSImport <- function(x, data, inputfiles, RetainedType,
                       TheSummary, stats) {
  # For each Fluorophore
  TheDetector <- data |>
    dplyr::filter(Fluorophore %in% x) |> pull(Detector)

  fcs_files <- inputfiles[str_detect(basename(inputfiles), x) &
    str_detect(basename(inputfiles), ".fcs$")]

  if (x %in% c("PE", "APC")) {
    x <- paste0(x, "-") #ExceptionHandling
  ...
  if (!length(fcs_files) == 0) {
    ...
    if (TheSummary == TRUE) {
      ...
      SmallHelper <- function(x, data, stats) {
        Cluster <- x
        Internal <- data |> dplyr::filter(Cluster %in% x) |>
