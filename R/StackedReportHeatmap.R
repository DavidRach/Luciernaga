...
#' @param nameColumn TBD
#' @param legend Default is "right", use "none"
#' @param transpose Default is FALSE, flips orientation
#'
#' @importFrom ggplot2 ggplot aes geom_tile geom_text coord_fixed theme_bw
#'  scale_fill_gradient theme element_blank element_text element_line
...
StackedReportHeatmap <- function(data, nameColumn, legend, transpose) {
  ...
  if (transpose == FALSE) {
    plot <- ggplot(data, aes(x = .data[[nameColumn]], y = Cluster,
                              fill = Ratio)) +
      geom_tile() +
    ...
    plot <- ggplot(data, aes(y = .data[[nameColumn]], x = Cluster,
                              fill = Ratio)) +
      geom_tile() +
      geom_text(aes(label = Ratio)) +
      theme_bw() +
