...
#'
#' @importFrom xml2 xml_children xml_name xml_text xml_find_all xml_attr
...
TubeIterate <- function(x) {
  ...
  InstrumentSN <- Landing[
    xml_name(Landing) == "data_instrument_serial_number"
  ]
  ...
  CytometerConfig <- xml_find_all(
    Keywords, ".//keyword[@name='CYTOMETER CONFIG NAME']"
  )
  ...
  if (!any(xml_name(Landing) == "instrument_settings")) {
    message(
      "Instrument Settings Node Absent for ", FileName, " returning NULL"
    )
    Dataset <- NULL
  } else {

    InstrumentSettings <- Landing[
      xml_name(Landing) == "instrument_settings"
    ][[1]]
    TheFluors <- xml_find_all(InstrumentSettings, ".//parameter[@name]")
    Fluorophores <- xml_attr(TheFluors, "name")

    FluorophoreGains <- map2(
      .x = TheFluors, .y = Fluorophores, .f = DivaParseInternal
    ) |>
      bind_rows()
    ...
    if (!any(xml_name(Landing) == "lasers")) {
      ...
      Lasers <- map2(
        .x = TheLaser_child, .y = Laser, .f = DivaLaserParseInternal
      ) |>
        bind_rows()
      ...
      LaserASF <- Lasers |>
        select(Laser, AreaScaling) |>
        pivot_wider(names_from = "Laser", values_from = "AreaScaling")
      colnames(LaserASF) <- paste0(
        colnames(LaserASF), "-Area Scaling Factor"
      )

      Dataset <- cbind(
        DateTime, User, FileName, Instrument,
