...
ReferenceScramble <- function(name, NumberDetectors, NumberFluors) {
  ...
  if (Spaces == 0) {
    message("Provide argument like `CD4 BUV805` for correct formatting")
  } else if (Spaces <= 1) {
    Name <- strsplit(name, " ")[[1]]
  } else {
    Name <- c(sub(" .*", "", name), sub("^[^ ]+ ", "", name))
  }
  Name <- Name[2]
  ...
  TheDetectorList <- TheDetectorGroups |>
    select(Detector) |>
    unique() |>
    pull()
  TheRandomDetectors <- sample(TheDetectorList, NumberFluors, replace = FALSE)
  TheDetectorGroups_subset <- TheDetectorGroups |>
    filter(Detector %in% TheRandomDetectors)
  TheSampling <- TheDetectorGroups_subset |>
    group_by(Detector) |>
    slice_sample(n = 1) |>
    ungroup()
  # TheSampling
