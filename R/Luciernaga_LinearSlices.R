...
Luciernaga_LinearSlices <- function(x, subset, sample.name,
                                     removestrings, stats, returntype,
                                     probsratio = 0.1, output,
                                     desiredAF = NULL, legend = TRUE,
                                     droplowest = TRUE, titlename = NULL,
  ...
  if (!is.null(parentSignature)) {
    n <- n |> mutate(across(everything(),
      ~ . - parentSignature[[cur_column()]]))
  }
  ...
  if (nrow(Detectors) > 1 && is.null(desiredAF)) {
    #message("Luciernaga_LinearSlices is only meant to work on
    # LuciernagaQC output .fcs files, your file ", name," contained
  ...
  if (nrow(Detectors) == 0) {
    stop("No Detectors Filtered at point 2! for sample ", name)
  }
  ...
  if (MultiDetector == TRUE) {
    ...
    if (nrow(data) == 0) {
      stop("We lost data at the filter step")
    }
  ...
  if (returncutplot == TRUE) {
    ...
    if (!is.null(titleplot)) {
      Title <- paste("Percentile Bins for ", TheDetector)
    } else {
      Title <- titleplot
    }
  ...
  if (returntype == "normalized") {
    ByPercentiles <- Normalized |>
      mutate(Percentiles = ByPercentiles$Percentiles)
  }
  ...
  if (output == "plot") {
    ...
    if (legend == TRUE) {
      plot <- ggplot(Melted, aes(x = Detector, y = value,
        group = Percentiles, color = Percentiles)) + geom_line() +
