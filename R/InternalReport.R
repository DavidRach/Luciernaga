...
#'   geom_tile scale_fill_gradient2 geom_col scale_fill_gradient
#'   coord_fixed geom_text aes theme_bw theme element_blank element_text
#'   ylim ggplot geom_line labs element_line
...
InternalReport <- function(x, data, FirstDetectorColumn,
  ...
  # TODO: if reactivated, the first %>% here must remain magrittr —
  #   the `.` placeholder sits nested inside select()/rowSums(), not
  #   in filter()'s own first-argument position, so |> cannot resolve it

  #if (ZeroBuggedRows > 0) {subset <- subset %>% filter(rowSums(select(
  #  ., all_of(First:Last)), na.rm = TRUE) != 0)}
  # TODO: if reactivated, this %>% must remain magrittr for the same
  #   nested `.` placeholder reason as above
  ...
  if (LinePlots == TRUE) {
    ...
    LinePlot <- ggplot(Melted, aes(x = Detector, y = value, group = Cluster,
      ...
      theme(plot.title = element_text(size = 16L, face = "plain",
              hjust = 0.5),
  ...
  if (CosinePlots == TRUE) {
    ...
    if (ncol(NumericsT) >= 2) {
      ...
      CosinePlot <- ggplot(MeltedCosine, aes(Var2, Var1, fill = value)) +
        ...
        scale_fill_gradient2(low = "lightblue", high = "orange",
                             mid = "white", midpoint = 0.7,
                             limit = c(0.4,1), space = "Lab",
                             name="Cosine\nSimilarity") +
        theme_bw() + geom_text(aes(Var2, Var1, label = value),
                               color = "black", size = 2) +
      ...
      image_path <- system.file("hex", "hex.png", package = "Luciernaga",
                                 mustWork = TRUE)
  ...
  if (exists("CosineOrder")) {
    Bd$Cluster <- factor(Bd$Cluster,
  ...
  if (StackedBarPlots == TRUE) {
    ...
    StackedBarPlot <- ggplot(Bd, aes(x= Sample, y = Ratio,
      fill = Cluster)) + geom_col() + theme_bw() +
