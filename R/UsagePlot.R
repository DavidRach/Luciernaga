...
#' @param TheInstrument Instrument designation in the Instrument column,
#'   example "5L"
...
#'
#' @importFrom dplyr filter group_by summarise arrange desc pull slice n
...
UsagePlot <- function(data, TheInstrument, returnType = "ByHour",
                      desiredfill = "black", textsizey = 10,
                      textsizex = 10) {
  ...
  if (returnType == "ByHour") {
    ...
    plot <- ggplot(dataBy15MinDay, aes(x = TimeNum, y = count)) +
      ...
      theme_bw() +
      scale_x_continuous(
        breaks = seq(0, 23.75, by = 1),
        labels = function(x) {
          sprintf("%02d:%02d", floor(x), (x - floor(x)) * 60)
        }
      ) +
      lims(y = c(NA, MaxCount)) +
      theme(
