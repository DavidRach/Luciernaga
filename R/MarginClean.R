...
#'   for (see colnames(gs)), default NULL cleans margins for all
#'   fluorophores
#' @param subset Subset at which to start margin cleanup, default is root.
#' @param themin Default is NULL, provide a numeric value to establish a
#'   lower boundary to clean up small debris.
#' @param themax Provide a numeric value to set the upper boundary to
#'   exclude margin events.
#' @param returnTemplate Default is FALSE, returns assembled openCyto gating
#'   template allowing for adjustments that can be brought in with
#'   importTemplate and inpath
#' @param importTemplate Default is FALSE, when TRUE, retrieves previous
#'   returnTemplate .csv from the inpath location and uses for the
#'   openCyto gating
#' @param returnPlots Default is FALSE, when TRUE, returns a pdf of margin
#'   clean events to the outpath to verify didn't cut of the population of
#'   interest.
#' @param Verbose Default is FALSE, will print to console the frequency of
#'   retained cells after margin cleanup for each specimen
#' @param inpath Default NULL, alternatively a file.path to the
#'   template.csv being imported for openCyto gating
#' @param outpath Default NULL, provide a file.path to desired location to
#'   store either the returnTemplate or the returnPlot objects
#' @param filename Default NULL, alternatively set a name for
#'   returnTemplate or returnPlot objects
#' @param yaxis Sets yaxis fluorophore on returnPlots, the default NULL
#'   utilizes the first fluorophore in the panel
#' @param sample.name Used when returningPlots, default NULL uses TUBENAME
#'   as the keyword value provided when retrieving sample name for plot
#'   titles.
...
MarginClean <- function(gs, desiredCols=NULL, subset="root",
 ...
 filename=NULL, yaxis=NULL, sample.name=NULL, inverse.transform=FALSE) {
  ...
  if (is.null(themin)) {
    ...
    thegatingargs <- paste0("min=", themin, ",max=", themax)
  }
  ...
  if (is.null(desiredCols)) {
    ...
    These <- TheCols[TheCols %in% desiredCols]
  }
  ...
  if (!importTemplate == TRUE) {
    ...
    Data <- map2(
      .x=thex, .y=they, .f=TemplateAssembly, template=Template) |>
      bind_rows()
    ...
    if (returnTemplate == TRUE) {
      ...
      if (!is.null(filename)) {
        ...
        TheFileName <- "MarginClean_template.csv"
      }
  ...
  if (returnPlots==TRUE) {
    ...
    if (!is.null(filename)) {
      TheFileName <- paste0(filename, "_plot.csv")
    } else {
      TheFileName <- "MarginClean_plot.csv"
    }
