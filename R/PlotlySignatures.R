...
PlotlySignatures <- function(data, TheFactor = "Fluorophore") {
  ...
  plot <- ggplot(Tidyed, aes(x = Detector, y = Value,
    color = .data[[TheFactor]], group = .data[[TheFactor]])) +
    geom_line() + labs(x = "Detector", y = "Normalized MFI",
      color = "Fluorophore") +
    theme_bw() + theme(axis.text.x = element_text(size = 5, angle = 70,
      hjust = 1))
