...
Utility_Subtract <- function(i, Data){
  #Generalization Note: This one may be a lot of effort generalize,
  #most of it is specific.

  Z <- Data[i,]
  Alpha <- Data %>% filter(Experiment == Z$Experiment) %>%
    filter(Type == Z$Type) %>% filter(Negative == Z$Negative) %>%
    filter(str_detect(Ligand, "Unstained"))
