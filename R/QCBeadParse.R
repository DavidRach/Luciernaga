  ...
  if (!length(FCS_Files) == 0) {
    ...
    if (length(QCBeads) == 0) {
      if (timepointType == "double") {
        ...
        message("No FCS Files detected")
      }
      QCBeads <- FCS_Files
    }

    BeforeAfter_CS <- tryCatch({
      load_cytoset_from_fcs(files = QCBeads, transformation = FALSE,
                             truncate_max_range = FALSE)
    }, error = function(e) {
      Screen <- CytosetScreen(files = QCBeads)
      MainList <- which.max(sapply(Screen, length))
      Screen <- Screen[MainList][[1]]
      TheSave <- load_cytoset_from_fcs(files = Screen, transformation = FALSE,
                                        truncate_max_range = FALSE)

      message("Following error occurred: ", e$message,
              " Attempted Rescue by passing main list.",
              " Please run Luciernaga:::CytosetScreen to find skipped",
              " files in the smaller list entries")
    ...
    if (timepointType == "double") {
      BeforeAfter <- map(.x = BeforeAfter_CS, .f = QC_GainMonitoring,
                          sample.name = "TUBENAME", stats = "median") |>
        bind_rows()
    } else {
      BeforeAfter <- map(.x = BeforeAfter_CS, .f = QC_GainMonitoring,
                          sample.name = "$DATE", stats = "median") |>
        bind_rows()
    }
