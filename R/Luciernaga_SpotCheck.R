...
#' @importFrom dplyr mutate select across arrange desc pull slice
...
Luciernaga_SpotCheck <- function(x, subsets, sample.name, CheckThese,
                                  inverse.transform = FALSE, Increments = 0.1,
                                  stats = "median") {
  ...
  # TODO: `.` inside nrow(.) refers to the piped data, but it's nested
  # inside mutate() rather than a top-level argument, so the native pipe's
  # `_` placeholder can't stand in for it here.
  OriginalColumnsIndex <- OriginalColumns %>%
    mutate(IndexLocation = 1:nrow(.))
