...
Utility_SingleColorQC <- function(x, subsets, sample.name, group.name,
  ...
  #Somewhat messy and Case-sensitive nightmare.
  ...
  InternalCleans <- c(" (Cells)", " (Beads)")
  name <- NameCleanUp(name, removestrings=InternalCleans)
  alternate.name <- name
  Cleanups <- c(" ", "(", ")", "_", "-")
  alternate.name <- NameCleanUp(alternate.name, removestrings=Cleanups)

  #Data Retrieval
  ff <- gs_pop_get_data(x, subsets)
  startingcells <- nrow(ff)[[1]] #TheProblemChild
  df <- exprs(ff[[1]])
