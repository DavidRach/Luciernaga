...
UserLibraryReturn <- function(x, Data, NameAppend, outpath, references=TRUE){
  ...
  TheGatheredData <- gather(TheUserData, key="Detector", value="value", all_of(5:columnLength))
  ...
  TheGatheredData <- TheGatheredData %>% mutate(TheSamples=paste0(Fluorochrome, "_", Sample, " ", Date)) %>%
  ...
  if(references==TRUE){
    ...
    ReferenceData$Fluorophore <- gsub(" ", "", gsub("-", "", gsub(".", "", fixed=TRUE, ReferenceData$Fluorophore)))
    #ReferenceFluorList <- ReferenceData %>% select(Fluorophore) %>% unique() %>% pull()
  }

  if (references == TRUE){ThePlots <- map(.x=TheSamples, .f=InternalLibraryPlot, Data=TheGatheredData, references=TRUE, refData=ReferenceData)
  } else {ThePlots <- map(.x=TheSamples, .f=InternalLibraryPlot, Data=TheGatheredData, references=FALSE, refData=NULL)
  }

  fileName <- paste(TheUser, NameAppend, sep="_")
  StorageLocation <- file.path(outpath, fileName)

  Utility_Patchwork(x=ThePlots, filename=fileName, outfolder=outpath, thecolumns=4, therows=4)
  ...
  InternalLibraryPlot <- function(x, Data, references=FALSE, refData=NULL){
    ...
    if (!references == TRUE){
      ...
      ThePlot <- ggplot(TheValue, aes(x=Detector, y=value, group=Sample)) +
        geom_line() + theme_bw() + labs(title=paste0(TheFluorochrome, " ", x), x=NULL, y="Normalized Value") +
      ...
      TheFluorochrome <- gsub("AF", "Alexa Fluor", gsub("efl", "eFl", gsub("Spk", "Spark", TheFluorochrome)))
      TheFluorochrome <- gsub(" ", "", gsub("-", "", gsub(".", "", fixed=TRUE, TheFluorochrome)))
      ReferenceFluorList <- refData %>% select(Fluorophore) %>% unique() %>% pull()

      if (any(ReferenceFluorList == TheFluorochrome)) { #TheCleanedVersion
        ReferenceData1 <- refData %>% filter(Fluorophore %in% TheFluorochrome) %>% rename(value=TheValue)
        ...
        ThePlot <- ThePlot + geom_line(data = ReferenceData1, aes(x=Detector, y=value, group=TheSamples), color="red")
...
UnknownFluorWhoseThis <- function(x, data, NumberHits) {
  ...
  Fluorophore <- StartingData %>% mutate(Fluorophore = paste0("ID_", Fluorochrome, "_", Sample)) %>% pull()
  ...
  ReferenceData <- ReferenceData %>% select(-Instrument) %>% group_by(Fluorophore) %>% pivot_wider(names_from = Detector, values_from = AdjustedY) %>% ungroup()
  ...
  TheHits <- TheData %>% filter(!Fluorophore %in% TheID) %>% arrange(desc(.data[[TheID]])) %>% slice_head(n=NumberHits)
  return(TheHits)
}
...
ReturnPlots <- function(x, data){
  ...
  TheNewData <- data %>% mutate(TheID = paste0("ID_", Fluorochrome, "_", Sample)) %>% relocate(TheID, .before=Fluorochrome)
  ...
  TheValue1 <- TheValue %>% select(where(is.numeric)) %>% mutate(TheID = TheValue$TheID) %>% relocate(TheID, .before=ncol(1)) %>% rename(Fluorophore=TheID)
  ...
  TheGatheredData <- gather(TheAssembly, key="Detector", value="value", all_of(2:columnLength))
  MyVector <- 1:columnLength
  TheGatheredData$Detector <- factor(TheGatheredData$Detector, levels=MyVector)

  ThePlot <- ggplot(TheGatheredData, aes(x=Detector, y=value, group=Fluorophore, color=Fluorophore)) +
    geom_line() + theme_bw() + labs(title=TheOGID, x=NULL, y="Normalized Value") +
    geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
    theme(plot.title = element_text(size = 8), axis.text.x = element_text(size = 6, angle = 45),
          panel.grid = element_blank(), axis.ticks.x = element_blank(), axis.title.y =  element_text(size=8)) +
    scale_x_discrete(breaks = unique(TheValue$Detector)[c(TRUE, rep(FALSE, 4))])
