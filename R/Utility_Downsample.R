...
#' @param internal Whether to return as a data.frame (vs. a flow frame or .fcs
#'   file)
#' @param export Default is set to FALSE, when TRUE (and internal = FALSE)
#'   returns a .fcs file to outpath.
#' @param outpath When export is true, the file.path to where you want the .fcs
#'   file stored.
#' @param metadataCols column names from pData to append as metadata for the
#'   .fcs, default NULL
#' @param addon Additional value to add before .fcs to denote modification,
#'   default NULL
#' @param removestrings TBD
#'
#' @importFrom flowWorkspace keyword gs_pop_get_data pData
#' @importFrom flowCore exprs parameters write.FCS
#' @importFrom dplyr slice_sample select slice mutate filter n
...
#'
#' removestrings <- c(
#'   "DTR_2023_ILT_15_Tetramers-", "-Ctrl_Tetramer_Unmixed", ".fcs"
#' )
...
#'   filter(
#'     Population %in%
#'       "/singletsFSC/singletsSSC/singletsSSCB/nonDebris/lymphocytes/live"
#'   ) |>
#'   select(name, Count)
#'
#' SingleSample <- Utility_Downsample(
#'   UnmixedGatingSet[1],
...
Utility_Downsample <- function(x, sample.name, subsets,
                               subsample = NULL, inverse.transform,
                               internal = FALSE, export = FALSE,
                               outpath = NULL, metadataCols = NULL,
                               addon = NULL) {
  ...
  if (internal == FALSE) {
    ...
    new_fcs <- new(
      "flowFrame",
      exprs = FCSSubset, parameters = original_p,
      description = original_d
    )

    if (export == TRUE) {
      if (!is.null(outpath)) {
        TheFileName <- paste0(alternatename, ".fcs")
    ...
    if (is.null(metadataCols)) {
      message(alternatename, " has been processed")
      DF <- DF |> mutate(specimen = alternatename)
      return(DF)
    } else {
      Repeats <- nrow(DF)
      # TODO: Native pipe |> passes left hand side as first arg.
      # Verify if dplyr::n() within slice works cleanly or if magrittr dot was needed.
      Metadata <- pData(x) |>
